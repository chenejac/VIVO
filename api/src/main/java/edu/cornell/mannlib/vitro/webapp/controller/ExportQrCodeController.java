...
public class ExportQrCodeController extends FreemarkerHttpServlet {
    ...
    protected ResponseValues processRequest(VitroRequest vreq) {
        try {
            ...
            body.put("individual",
                wrapper.wrap(IndividualTemplateModelBuilder.build(individual, vreq)));
    ...
    private Map<String, String> generateQrData(Individual individual, VitroRequest vreq) {
        ...
        try {
            ...
            String prefix = "http://";
            tempUrl =
                tempUrl.substring(0, tempUrl.replace(prefix, "").indexOf("/") + prefix.length());
            ...
            qrData.put("exportQrCodeUrl",
                contextPath + "/qrcode?uri=" + UrlBuilder.urlEncode(individualUri));
    ...
    private List<Map<String, String>> getVcardData(Individual individual, VitroRequest vreq) {
        String queryStr =
            QueryUtils.subUriForQueryVar(VCARD_DATA_QUERY, "subject", individual.getURI());

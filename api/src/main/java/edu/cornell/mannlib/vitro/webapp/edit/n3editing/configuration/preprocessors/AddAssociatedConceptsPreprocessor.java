...
public class AddAssociatedConceptsPreprocessor extends
    ...
    private void addConceptSemanticTypeURIResources(int numberConcepts) {
        ...
        String[] conceptSemanticTypeLabels =
            convertDelimitedStringToArray(conceptSemanticTypeLabelValues);
        ...
        if (conceptSemanticTypeLabels != null &&
            conceptSemanticTypeLabels.length <= numberConcepts) {
            int i;
            for (i = 0; i < numberConcepts; i++) {
                ...
                if (this.labelVarToUriVarHash.containsKey(conceptSemanticTypeLabelVar)) {
                    String newResourceName =
                        this.labelVarToUriVarHash.get(conceptSemanticTypeLabelVar);
        ...
        } else if (conceptSemanticTypeLabels != null &&
            conceptSemanticTypeLabels.length > numberConcepts) {
    ...
    private void addConceptLabelInputs(int numberConcepts) {
        ...
        if (labels != null && labels.length == numberConcepts) {
            ...
            for (i = 0; i < numberConcepts; i++) {
                ...
                if (labelField != null) {
                    submission.addLiteralToForm(editConfiguration, labelField, labelInputName,
                        labelValues);
                } else {
                    log.error("Corresponding field for " + labelInputName +
                        " was not added to edit configuration");
                }
    ...
    private void addConceptSemanticTypeLabelAndURIInputs(int numberConcepts) {
        ...
        if (labels != null && labels.length == numberConcepts) {
            ...
            log.error(
                "Number of concept semantic type labels did not match the number of concepts to be added");
        } else {
            log.error("Concept labels returned were null");
        }
    }

    private void addConceptSemanticTypeURIInputForLabel(String conceptSemanticTypeLabel,
                                                        int suffix) {
        ...
        String uriInputName =
            this.getConceptSemanticTypeURIFieldName(conceptSemanticTypeLabel, suffix);
        //List<>
    ...
    private void addConceptBroaderURIInputs(int numberConcepts) {
        ...
        if (this.conceptBroaderURIValues.size() > 0 &&
            this.conceptBroaderURIValues.size() <= numberConcepts) {
            for (i = 0; i < numberConcepts; i++) {
                int suffix = i + 1;
                ...
                if (broaderURIs != null && !broaderURIs.isEmpty()) {
                    ...
                    //Add value for uri to form
                    submission
                        .addUriToForm(editConfiguration, conceptBroaderURIInputName, broaderURISet);
                }
            }
        }
    ...
    private void addConceptNarrowerURIInputs(int numberConcepts) {
        int i;
        if (this.conceptNarrowerURIValues.size() > 0 &&
            this.conceptNarrowerURIValues.size() <= numberConcepts) {
            for (i = 0; i < numberConcepts; i++) {
                int suffix = i + 1;
                ...
                if (narrowerURIs != null && !narrowerURIs.isEmpty()) {
                    ...
                    //Add value for uri to form
                    submission.addUriToForm(editConfiguration, conceptNarrowerURIInputName,
                        narrowerURISet);
                }
            }
        }
    ...
    private void addN3Optional(int numberConcepts) {
        ...
        for (index = 1; index <= numberConcepts; index++) {
            ...
            String conceptSemanticTypeURI =
                getConceptSemanticTypeURIVar(conceptSemanticTypeLabelBase + suffix, suffix);
            ...
            n3ConceptTypeString +=
                node + " <" + RDF.type.getURI() + "> " + conceptSemanticTypeURI + " ." +
                    conceptSemanticTypeURI + " <" + RDFS.label.getURI() + "> " +
                    conceptSemanticTypeLabel + " .\n" +
            ...
            String n3ConceptBroaderURI =
                prefixStr + node + " <" + this.SKOSNarrowerURI + "> " + conceptNarrowerURI + " ." +
                    conceptNarrowerURI + " <" + this.SKOSBroaderURI + "> " + node + " .";
            String n3ConceptNarrowerURI =
                prefixStr + node + " <" + this.SKOSBroaderURI + "> " + conceptBroaderURI + " ." +
    ...
    private String getConceptLabelValues() {
        ...
        Map<String, List<String>> transformed =
            EditConfigurationUtils.transformLiteralMap(literalsFromForm);
    ...
    private String getConceptSemanticTypeURIValues() {
        ...
        if (conceptSemanticTypeLabelValues != null && !conceptSemanticTypeLabelValues.isEmpty()) {
            ...
            HashMap<String, List<Integer>> labelValueToVarSuffix =
            ...
            if (numberLabels == 1) {
                ...
                if (!StringUtils.isEmpty(uri)) {
                    ...
                    submission
                        .addUriToForm(this.editConfiguration, "conceptSemanticTypeURI", urisToAdd);
                }
                ...
                for (int i = 0; i < numberLabels; i++) {
                    ...
                    if (!StringUtils.isEmpty(uri)) {
                        ...
                        if (!this.conceptSemanticTypeURIVarToValueMap.containsKey(uriVar)) {
                            this.conceptSemanticTypeURIVarToValueMap
                                .put(uriVar, new ArrayList<String>());
    ...
    private String getExistingSemanticTypeURI(String label) {
        String queryStr =
            "SELECT ?semanticType WHERE { ?semanticType <" + RDF.type.getURI() + "> <" +
                OWL.Class.getURI() + "> . " +
        ...
        try {
            ...
            throw new Error(
                "Error in executing query string: \n" + queryStr + '\n' + ex.getMessage());
        } finally {
            if (qe != null) {
                qe.close();
            }

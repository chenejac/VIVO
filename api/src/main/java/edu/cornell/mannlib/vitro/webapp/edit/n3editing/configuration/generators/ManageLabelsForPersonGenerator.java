 ...
 */
public class ManageLabelsForPersonGenerator extends BaseEditConfigurationGenerator
    ...
    private void setUrlToReturnTo(EditConfigurationVTwo editConfiguration, VitroRequest vreq) {
        editConfiguration
            .setUrlPatternToReturnTo(EditConfigurationUtils.getFormUrlWithoutContext(vreq));
    }
    ...
    private List<String> generateN3Optional(VitroRequest vreq) {
        ...
        String personMiddleNameN3 =
            "?subject <http://vivoweb.org/ontology/core#middleName> ?middleName .";
    ...
    private void addFormSpecificData(EditConfigurationVTwo config,
        ...
        HashMap<String, List<LabelInformation>> existingLabelsByLanguageName =
            this.getLabelsSortedByLanguageName(existingLabels, localeCodeToNameMap, config, vreq);
        //Get available locales for the drop down for adding a new label, also sorted by language name
        HashSet<String> existingLanguageNames =
            new HashSet<String>(existingLabelsByLanguageName.keySet());
        List<HashMap<String, String>> availableLocalesForAdd =
            getAvailableLocales(locales, existingLanguageNames);
        ...
        for (HashMap<String, String> tmp : availableLocalesForAdd) {
            if (tmp.get("code").equals(rangeLang)) {
                localeEntryExisting = false;
            }
        }
        ...
        Individual subject = vreq.getWebappDaoFactory().getIndividualDao()
            .getIndividualByURI(config.getSubjectUri());
    ...
    private List<HashMap<String, String>> getAvailableLocales(
        List<HashMap<String, String>> allLocales,
        ...
        for (HashMap<String, String> localeInfo : allLocales) {
            ...
            if (!"untyped".equals(languageName) &&
                !existingLabelsLanguageNames.contains(languageName)) {
    ...
    private HashMap<String, List<LabelInformation>> getLabelsSortedByLanguageName(
        List<Literal> labels, Map<String, String> localeCodeToNameMap, EditConfigurationVTwo config,
        VitroRequest vreq) {
        ...
        HashMap<String, List<LabelInformation>> labelsHash =
            new HashMap<String, List<LabelInformation>>();
        ...
        for (Literal l : labels) {
            ...
            if (!StringUtils.isEmpty(languageName)) {
                ...
                ArrayList<LabelInformation> labelsList =
                    (ArrayList<LabelInformation>) labelsHash.get(languageName);
                //This should put the label in the list
                //Create label information instance with the required information
                //To generate link

                DataPropertyStatementTemplateModel dpstm =
                    new DataPropertyStatementTemplateModel(subjectUri, prop, l,
                        template, vreq);
    ...
    public HashMap<String, String> getFullCodeToLanguageNameMap(
        ...
        for (Map<String, String> locale : localesList) {
            ...
            if (!codeToLanguageMap.containsKey(code)) {
                ...
                log.warn("Language code " + code + " for " + label +
                    " was not associated in map becayse label already exists");
            }
        }
    ...
    public class LabelInformation {
        ...
        public LabelInformation(Literal inputLiteral, String inputEditLinkURL,
                                String inputDeleteLinkURL, String inputLanguageCode,

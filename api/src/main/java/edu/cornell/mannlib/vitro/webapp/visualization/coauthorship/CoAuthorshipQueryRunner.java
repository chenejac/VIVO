...
public class CoAuthorshipQueryRunner implements QueryRunner<CoAuthorshipData> {
    ...
    private static final Map<String, CollaborationDataCacheEntry> collaborationDataCache =
        new ConcurrentHashMap<String, CollaborationDataCacheEntry>();
    ...
    private synchronized CoAuthorshipData getQueryResultAndCache()
        ...
        if (preferCaches && VisualizationCaches.personToPublication.isCached()) {
            ...
            Map<String, Set<String>> publicationToPersonMap =
                VisualizationCaches.personToPublication.get(rdfService).publicationToPerson;
            Map<String, String> publicationToYearMap =
                VisualizationCaches.publicationToYear.get(rdfService);
            ...
            for (String documentURI : personToPublicationMap.get(authorURI)) {
                ...
                for (String coAuthorURI : publicationToPersonMap.get(documentURI)) {
                    ...
                    consumer
                        .processEntry(authorURI, authorName, documentURI, documentDate, coAuthorURI,
                            coAuthorName);
                }
            }
            ...
            try {
                ...
                rdfService.sparqlConstructQuery(generateEgoCoAuthorshipSparqlConstruct(this.egoURI),
                    constructedModel);
                QueryExecution qe = QueryExecutionFactory
                    .create(generateEgoCoAuthorshipSparqlQuery(this.egoURI), constructedModel);
                try {
    ...
    private Collaborator makeEgoNode() {
        ...
        collab
            .setCollaboratorName(UtilityFunctions.getIndividualLabelFromDAO(vitroRequest, egoURI));
    ...
    private static class QueryResultConsumer extends ResultSetConsumer {
        ...
        protected void processQuerySolution(QuerySolution qs) {
            ...
            String authorURI =
                egoAuthorURLNode == null ? null : egoAuthorURLNode.asLiteral().getString();
            String authorName =
                authorLabelNode == null ? null : authorLabelNode.asLiteral().getString();
            String documentURI = documentNode == null ? null : documentNode.asLiteral().getString();
            String documentDate =
                publicationDateNode == null ? null : publicationDateNode.asLiteral().getString();
            String coAuthorURI =
                coAuthorURLNode == null ? null : coAuthorURLNode.asLiteral().getString();
            String coAuthorName =
                coAuthorLabelNode == null ? null : coAuthorLabelNode.asLiteral().getString();

            processEntry(authorURI, authorName, documentURI, documentDate, coAuthorURI,
                coAuthorName);
        }

        public void processEntry(String authorURI, String authorName, String documentURI,
                                 String documentDate, String coAuthorURI, String coAuthorName) {
            /*

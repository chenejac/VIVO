 ...
 */
@WebServlet(name = "InstitutionalInternalClassController", urlPatterns = {
    "/processInstitutionalInternalClass"})
public class InstitutionalInternalClassController extends FreemarkerHttpServlet {
    public final static AuthorizationRequest REQUIRED_ACTIONS =
    ...
    private void retrieveLocalClasses(VitroRequest vreq, Map<String, Object> data) {
        ...
        localNamespaceClasses =
            LocalNamespaceClassUtils.getLocalNamespacesClasses(vreq, localNamespaces);
        data.put("existingLocalClasses", localNamespaceClasses);
        data.put("existingLocalNamespaces", localNamespaces);
        String noLocalOntologiesMessage =
            "There are currently no local ontologies.  You must create a new ontology";
    ...
    private void processSubmission(VitroRequest vreq, Map<String, Object> data) {
        ...
        if (isNewClassSubmission(vreq)) {
            VClass v = generateNewVClass(vreq.getParameter("localClassName"),
                vreq.getParameter("existingLocalNamespaces"));
            classUri = v.getURI();
            try {
                vreq.getWebappDaoFactory().getVClassDao().insertNewVClass(v);
            } catch (Exception ex) {
                log.error(
                    "Insertion of new class " + vreq.getParameter("name") + " resulted in error ",
                    ex);
            }
        } else {
        ...
        if (classUri != null && !classUri.isEmpty()) {
            ...
            try {
                ...
                writeModel.removeAll(null,
                    ResourceFactory.createProperty(VitroVocabulary.IS_INTERNAL_CLASSANNOT),
                    (RDFNode) null);
                log.debug("Are there any statements left for internal class annotation:  " +
                ...
                log.error("Error occurred in adding statement for " + classUri +
                    " becoming internal class", ex);
            } finally {
                writeModel.notifyEvent(new EditEvent(null, true));

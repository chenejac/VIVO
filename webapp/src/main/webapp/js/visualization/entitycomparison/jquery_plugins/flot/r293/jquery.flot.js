...
(function ($) {
    ...
    $.color.make = function (E, D, B, C) {
        ...
        F.clone = function () {
            return $.color.make(F.r, F.b, F.g, F.a)
        };
        return F.normalize()
    };
    $.color.extract = function (C, B) {
        var D;
        do {
            ...
            C = C.parent()
        } while (!$.nodeName(C.get(0), "body"));
        if (D == "rgba(0, 0, 0, 0)") {
            D = "transparent"
        }
        return $.color.parse(D)
    };
    $.color.parse = function (E) {
        var D, B = $.color.make;
